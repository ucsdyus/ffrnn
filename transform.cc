...
bool inside_grid(int i, int j, int k) {
    return i >= 0 && j >= 0 && k >= 0 && i < KERNEL_SIZE && j < KERNEL_SIZE && k < KERNEL_SIZE;
}

float trilinear_w(float d, int b) {
    return b * d + (1 - b) * (1 - d);
}

float window_smooth_weight(float* r) {
...
void ball2cyl(float x, float y, float z, float& rx, float& ry, float& rz) {
    float x2_y2 = x * x + y * y;
    float z2 = z * z;
...
void cyl2cube(float x, float y, float z, float& rx, float& ry, float& rz) {
    if (x == 0 && y == 0) {
        rx = ry = 0;
        rz = z;
        return;
...
void ball2cube(float* r_ptr, float* h_ptr) {
    float rx, ry, rz;
...
void weighted_ball2grid(float* r_ptr, float* grid_ptr, float smooth_weight) {
    float h[3];
    ball2cube(r_ptr, h);
...
void ball2grid_with_window(float* r_ptr, float* grid_ptr) {
...
torch::Tensor th_ball2cube(torch::Tensor r) {
    torch::Tensor cube = torch::zeros_like(r);
    ball2cube(r.data_ptr<float>(), cube.data_ptr<float>());
    return cube;
}

torch::Tensor th_weighted_ball2grid(torch::Tensor r, float smooth_weight) {
...
torch::Tensor th_ball2grid(torch::Tensor r) {
    return th_weighted_ball2grid(r, /*smooth_weight=*/1.0f);
}

torch::Tensor th_ball2grid_with_window(torch::Tensor r) {
